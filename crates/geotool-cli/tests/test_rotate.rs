...
static A1: LazyLock<f64> = LazyLock::new(|| {
    let radians = ANGLE.to_radians();
    (A - OA) * radians.cos() - (B - OB) * radians.sin() + OA
});
static B1: LazyLock<f64> = LazyLock::new(|| {
    let radians = ANGLE.to_radians();
    (A - OA) * radians.sin() + (B - OB) * radians.cos() + OB
});
#[test]
...
fn test_rotate_xy() {
        ...
        Command::cargo_bin("geotool")
            ...
            .args(["transform", "-x", "2", "-y", "4", "-z", "6"])
            .args([
                "rotate", "--value", "150", "-p", "xy", "-u", "radians", "--ox", "2", "--oy", "4",
                "--oz", "6",
            ])
            .assert()
            .success()
