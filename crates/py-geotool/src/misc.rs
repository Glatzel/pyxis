...
pub fn py_datum_compense(
    ...
    let parms = geotool_algorithm::DatumCompenseParms::new(hb, r, x0, y0);
    if let (Ok(xc_ref), Ok(yc_ref)) = (
        ...
        xc_array
            ...
            .for_each(|(x, y)| {
                (*x, *y) = geotool_algorithm::datum_compense(*x, *y, &parms);
            });
        (xc_ref, yc_ref).into_pyobject(py)
    } else if let (Ok(xc), Ok(yc)) = (xc_py.extract::<f64>(py), yc_py.extract::<f64>(py)) {
        geotool_algorithm::datum_compense(xc, yc, &parms).into_pyobject(py)
    } else {
