...
impl IPjCoord for (f64, f64, f64, f64) {
    ...
    fn pj_y(&mut self) -> *mut f64 { &mut self.1 }
    fn pj_z(&mut self) -> *mut f64 { &mut self.2 }
    fn pj_t(&mut self) -> *mut f64 { &mut self.3 }
    fn to_array4(self) -> [f64; 4] { [self.0, self.1, self.2, self.3] }
    fn from_pj_coord(x: f64, y: f64, z: f64, t: f64) -> Self { (x, y, z, t) }
}
