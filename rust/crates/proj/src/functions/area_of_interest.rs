...
impl crate::Area {
    ...
    pub fn set_bbox(
        ...
        check_result!(
            !(-90.0..=90.0).contains(&south_lat_degree),
            "`south_lat_degree ` should in [-90,90] range."
        );
        check_result!(
            !(-180.0..=180.0).contains(&east_lon_degree),
            "`east_lon_degree` should in [-180,180] range."
        );
        check_result!(
            !(-90.0..=90.0).contains(&north_lat_degree),
            "`north_lat_degree ` should in [-90,90] range."
        );
        unsafe {
