    ...
    pub fn get_celestial_body_list_from_database(
        ...
        for offset in 0..out_result_count {
            let current_ptr = unsafe {
                ptr.offset(offset as isize).as_ref().ok_or_else(|| {
                    ProjError::new("Invalid celestial body info pointer".to_string())
                })?
            };
            let info_ref = unsafe {
                current_ptr.as_ref().ok_or_else(|| {
                    ProjError::new("Invalid celestial body info pointer".to_string())
                })?
            };
    ...
    pub fn get_units_from_database(
        ...
        for offset in 0..out_result_count {
            ...
            out_vec.push(UnitInfo::new(
                ...
                info_ref.code.to_string()?,
                info_ref.name.to_string()?,
                UnitCategory::from_str(&info_ref.category.to_string()?)?,
