...
impl crate::Context {
    ...
    /// # Arguments
    ///
    /// \* `authority`: to restrict the authority of coordinate operations
    /// looked   up in the database. When not specified, coordinate
    /// operations from any   authority will be searched, with the
    /// restrictions set in the   authority_to_authority_preference database
    /// table related to the   authority of the source/target CRS
    /// themselves. If authority is set to   any, then coordinate operations
    /// from any authority will be searched.   If authority is a non-empty
    /// string different of any, then coordinate   operations will be
    /// searched only in that authority namespace (e.g   EPSG).
    ...
    /// out   if the best transformation, known of PROJ, and usable by PROJ
    /// if all   grids known and usable by PROJ were accessible, cannot be
    /// used. Best   transformation should be understood as the
    /// transformation returned by   proj_get_suggested_operation() if all
    /// known grids were accessible   (either locally or through network).
    /// Note that the default value for   this option can be also set with
