...
pub fn check_port() ->Result<(),ProjError> {
    let (port,) = {
        ...
        .any(|p| p.port_name.eq_ignore_ascii_case(&port))
    {
        ...
        if !cfg!(debug_assertions) {
            ProjError{code:ProjErrorCode::Other,message:"{msg}".to_string()};
        }
    }
    Ok(())
}
pub async fn start_serial_reader(tx: Sender<(Talker, Identifier, String)>) ->Result<(),ProjError> {
    let (port, baud_rate) = {
        let settings = crate::settings::SETTINGS.lock();
