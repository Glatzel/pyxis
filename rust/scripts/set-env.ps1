...
if ($IsWindows) {
    ...
    if ($link -eq 'static') {
        $env:PKG_CONFIG_PATH = Resolve-Path "./vcpkg/vcpkg/installed/static/x64-windows-static/lib/pkgconfig"
    }
    else {
        $env:PKG_CONFIG_PATH = Resolve-Path "./vcpkg/vcpkg/installed/dynamic/x64-windows/lib/pkgconfig"
    }
