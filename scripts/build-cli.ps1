...
if ($IsWindows) {
    ...
    if ($config -ne "debug") {
        pixi run cargo build --profile $config -p pyxis-cli --features static
    }
    else {
        pixi run cargo build -p pyxis-cli --features static
    }
    ...
    7z a -t7z -m0=LZMA2 -mmt=on -mx9 -md=4096m -mfb=273 -ms=on -mqs=on `
        "dist/pyxis-windows-x64-self-contained.7z" "./dist/cli/static/pyxis.exe"
    Write-Output "::endgroup::"

    Write-Output "::group::Build dynamic"
    & $PSScriptRoot/set-env.ps1 -link dynamic
    if ($config -ne "debug") {
        pixi run cargo build --profile $config -p pyxis-cli
    }
    else {
        pixi run cargo build -p pyxis-cli
    }
    ...
    7z a -t7z -m0=LZMA2 -mmt=on -mx9 -md=4096m -mfb=273 -ms=on -mqs=on `
        "dist/pyxis-windows-x64.7z" "./dist/cli/dynamic/pyxis.exe"
    Write-Output "::endgroup::"
...
elseif ($IsLinux) {
    ...
    7z a -t7z -m0=LZMA2 -mmt=on -mx9 -md=4096m -mfb=273 -ms=on -mqs=on `
        "dist/pyxis-linux-x64-self-contained.7z" "./dist/cli/static/*"
    Write-Output "::endgroup::"
}
else {
