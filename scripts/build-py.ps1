...
Remove-Item src/rust/pyxis-py/pyxis/pyxis_py.pyd -ErrorAction SilentlyContinue
Remove-Item src/rust/pyxis-py/pyxis/**__pycache__ -Recurse -ErrorAction SilentlyContinue

Write-Host "Build in $config mode."
if ($config -ne "debug") {
    pixi run cargo build --profile $config -p pyxis-py
    Set-Location src/rust/pyxis-py
