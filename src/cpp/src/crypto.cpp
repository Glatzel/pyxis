    ...
    T &d_lon, T &d_lat)
{
    ...
    transform<T>(lon - 105.0, lat - 35.0, d_lon, d_lat);
    ...
    T &wgs84_lon, T &wgs84_lat)
{
    T d_lon = 0.0;
    T d_lat = 0.0;
    delta<T>(gcj02_lon, gcj02_lat, d_lon, d_lat);
    ...
    T &gcj02_lon, T &gcj02_lat)
{
    T d_lon = 0.0;
    T d_lat = 0.0;
    delta<T>(wgs84_lon, wgs84_lat, d_lon, d_lat);
                                      ...
                                      const T lon_b, const T lat_b)
{
    ...
    T lat1_rad = to_radians<T>(lat_a);
    T lon1_rad = to_radians<T>(lon_a);
    T lat2_rad = to_radians<T>(lat_b);
    T lon2_rad = to_radians<T>(lon_b);
